...
export default function ProjectManager({ userId }: { userId: string }) {
  ...
  const resetForm = () => {
    setFormTitle(''); setFormDesc(''); setFormOutcomes('')
    setFormTechStack(''); setFormDocIds([]); setFormLinks([])
    setFormLinkLabel(''); setFormLinkUrl('')
  }

  const startEdit = (p: Project) => {
    setFormTitle(p.title)
    setFormDesc(p.description || '')
    setFormOutcomes(p.outcomes || '')
    setFormTechStack((p.tech_stack || []).join(', '))
    setFormDocIds(p.doc_ids || [])
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/profile/projects`, {
        ...
        body: JSON.stringify({
          ...
          outcomes:    formOutcomes.trim() || null,
          tech_stack:  formTechStack ? formTechStack.split(',').map(s => s.trim()).filter(Boolean) : [],
          doc_ids:     formDocIds,
          links:       formLinks,
        }),
      })
  ...
  const handleSaveEdit = async (projectId: string) => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/profile/projects/${projectId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          title:       formTitle.trim(),
      ...
      const data = await res.json()
      if (data.project) {
        setProjects(prev => prev.map(p => p.project_id === projectId ? data.project : p))
        setEditingId(null)
        resetForm()
      }
    } catch {}
  ...
  const handleSynthesize = async (projectId: string) => {
    ...
    try {
      ...
      if (data.result?.summary) {
        setProjects(prev => prev.map(p =>
          p.project_id === projectId ? { ...p, synthesized_summary: data.result.summary } : p
        ))
      }
    } catch {}
  ...
  const addLink = () => {
    ...
    setFormLinks(prev => [...prev, { label: formLinkLabel || formLinkUrl, url: formLinkUrl }])
    setFormLinkLabel(''); setFormLinkUrl('')
  }

  const toggleDoc = (docId: string) => {
    setFormDocIds(prev => prev.includes(docId) ? prev.filter(d => d !== docId) : [...prev, docId])
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      <div>
        ...
        <textarea value={formDesc} onChange={e => setFormDesc(e.target.value)}
          placeholder="What was this project?" rows={2} style={{ ...inputStyle, resize: 'vertical' }} />
      </div>
      <div>
        <Label text="OUTCOMES & METRICS" />
        <textarea value={formOutcomes} onChange={e => setFormOutcomes(e.target.value)}
          placeholder="e.g. Improved CSAT from 3.9 to 8.7, 85,000 users impacted" rows={2}
          style={{ ...inputStyle, resize: 'vertical' }} />
      </div>
      <div>
        <Label text="SKILLS / TECH USED (COMMA SEPARATED)" />
        <input value={formTechStack} onChange={e => setFormTechStack(e.target.value)}
          placeholder="e.g. Python, Stakeholder Management, Tableau, Salesforce" style={inputStyle} />
      </div>

      {vaultDocs.length > 0 && (
        <div>
          <Label text="ATTACHED DOCUMENTS FROM VAULT" />
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            {vaultDocs.map(doc => (
              <div key={doc.doc_id} onClick={() => toggleDoc(doc.doc_id)} style={{
                display: 'flex', alignItems: 'center', gap: '8px',
                padding: '6px 10px', borderRadius: '6px', cursor: 'pointer',
                background: formDocIds.includes(doc.doc_id) ? 'rgba(16,185,129,0.08)' : 'rgba(255,255,255,0.02)',
                border: `1px solid ${formDocIds.includes(doc.doc_id) ? 'rgba(16,185,129,0.25)' : BORDER}`,
              }}>
                <span style={{ fontSize: '11px', color: formDocIds.includes(doc.doc_id) ? TEAL : 'rgba(255,255,255,0.4)' }}>
                  {formDocIds.includes(doc.doc_id) ? '✓' : '○'}
                </span>
      ...
      <div>
        ...
        {formLinks.map((l, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
            ...
            <button onClick={() => setFormLinks(prev => prev.filter((_, j) => j !== i))}
              style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.2)', cursor: 'pointer', fontSize: '12px' }}>✕</button>
          </div>
        ))}
        <div style={{ display: 'flex', gap: '6px' }}>
          <input value={formLinkLabel} onChange={e => setFormLinkLabel(e.target.value)}
            placeholder="Label (e.g. GitHub)" style={{ ...inputStyle, flex: 1 }} />
          <input value={formLinkUrl} onChange={e => setFormLinkUrl(e.target.value)}
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        <button onClick={onSave} disabled={!formTitle.trim() || saving} style={{
          padding: '8px 16px', borderRadius: '8px',
          background: formTitle.trim() ? TEAL : 'rgba(255,255,255,0.06)',
          color: formTitle.trim() ? '#fff' : 'rgba(255,255,255,0.3)',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      {projects.map(p => (
        <div key={p.project_id} style={{
          ...
          <div style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}
            onClick={() => { if (editingId !== p.project_id) setExpandedId(expandedId === p.project_id ? null : p.project_id) }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
                {p.tech_stack?.slice(0, 3).map(t => (
                  <span key={t} style={{ fontSize: '10px', padding: '1px 6px', borderRadius: '4px', background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.4)' }}>{t}</span>
                ))}
                {(p.tech_stack?.length || 0) > 3 && (
                  <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.25)' }}>+{p.tech_stack.length - 3} more</span>
                )}
                {p.doc_ids?.length > 0 && (
                  <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.25)' }}>📎 {p.doc_ids.length} doc{p.doc_ids.length > 1 ? 's' : ''}</span>
                )}
              </div>
            </div>
              ...
              {editingId !== p.project_id && (
                <>
                  ...
                  {!p.synthesized_summary && (
                    ...
                    >
                      {synthesizing === p.project_id ? '...' : '✨ Generate summary'}
                    </button>
                  )}
                  <button
                    onClick={e => { e.stopPropagation(); startEdit(p) }}
                    style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', fontSize: '13px', padding: '2px' }}
                    title="Edit project"
                  >✎</button>
                  <button
                    onClick={e => { e.stopPropagation(); handleDelete(p.project_id) }}
                    style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.2)', cursor: 'pointer', fontSize: '14px', padding: '2px' }}
                  >🗑</button>
                  <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.2)' }}>{expandedId === p.project_id ? '▴' : '▾'}</span>
                </>
              )}
            </div>
          </div>
          ...
          {/* Expanded — view mode */}
          {expandedId === p.project_id && editingId !== p.project_id && (
            <div style={{ padding: '0 14px 14px', borderTop: `1px solid ${BORDER}` }}>
              {p.synthesized_summary && (
                <div style={{ padding: '10px 12px', borderRadius: '8px', margin: '12px 0', background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.15)' }}>
                  <p style={{ fontSize: '10px', fontWeight: 700, color: TEAL, margin: '0 0 4px', letterSpacing: '0.07em' }}>✨ AI SUMMARY</p>
                  <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.7)', margin: 0, lineHeight: 1.6 }}>{p.synthesized_summary}</p>
                  <button
                    onClick={() => handleSynthesize(p.project_id)}
                    disabled={synthesizing === p.project_id}
                    style={{ marginTop: '6px', fontSize: '10px', color: 'rgba(255,255,255,0.3)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
                  >
              ...
              {p.tech_stack?.length > 0 && (
                <div style={{ marginTop: '10px' }}>
                  <p style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.07em', margin: '0 0 6px' }}>SKILLS / TECH USED</p>
                  <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                    {p.tech_stack.map(t => (
                      <span key={t} style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '4px', background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.5)' }}>{t}</span>
                    ))}
                  </div>
                </div>
              )}
                ...
                <div style={{ marginTop: '10px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    {p.doc_ids.map(docId => {
                      ...
                      return doc ? <p key={docId} style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', margin: 0 }}>📄 {doc.file_name}</p> : null
                    })}
                  </div>
                </div>
              )}

              {!p.description && !p.outcomes && !p.tech_stack?.length && (
                <div style={{ marginTop: '12px', padding: '10px 12px', borderRadius: '8px', background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.15)' }}>
                  <p style={{ fontSize: '12px', color: '#F59E0B', margin: 0 }}>
                    This project needs more details. Click ✎ to add a description, outcomes, and skills used.
                  </p>
                </div>
              )}
            </div>
      ...
      {/* Add project form */}
      {adding && (
        <div style={{ padding: '16px', borderRadius: '12px', background: 'rgba(255,255,255,0.02)', border: `1px solid ${BORDER}` }}>
          {renderForm(handleAdd, () => { setAdding(false); resetForm() })}
        </div>
      )}

      {!adding && (
        <button onClick={() => { setAdding(true); setEditingId(null); resetForm() }} style={{
          padding: '8px 14px', borderRadius: '8px',
          background: 'rgba(255,255,255,0.03)', border: `1px solid ${BORDER}`,
          color: 'rgba(255,255,255,0.4)', fontSize: '12px', cursor: 'pointer', textAlign: 'left',
        }}>
      ...
      <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.2)', margin: 0, lineHeight: 1.5 }}>
        Career Sage automatically selects the most relevant projects when generating your resume for each job.
      </p>
    </div>
  )
}