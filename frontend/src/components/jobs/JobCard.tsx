      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0, marginLeft: '12px' }}>
            ...
            <button onClick={handleStar} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', padding: '2px', color: starred ? '#F59E0B' : 'rgba(255,255,255,0.2)', transition: 'color 0.15s' }}>
              {starred ? '★' : '☆'}
            </button>
          </div>
        </div>
          ...
          <div style={{ marginBottom: '10px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
              {job.identified_skill_gaps.map(s => (
                <span key={s} style={{ fontSize: '11px', padding: '2px 7px', borderRadius: '4px', background: 'rgba(239,68,68,0.08)', color: 'rgba(239,68,68,0.7)', border: '1px solid rgba(239,68,68,0.15)' }}>{s}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {job.job_description && (
          <div style={{ marginBottom: '12px' }}>
            <button onClick={() => setExpanded(!expanded)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '12px', color: 'rgba(255,255,255,0.35)', padding: 0, textDecoration: 'underline' }}>
              {expanded ? 'Hide description' : 'Show description'}
            </button>
            {expanded && (
              <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, margin: '8px 0 0' }}>
                {job.job_description}
              </p>
            )}
          </div>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button
            ...
            style={{ flex: 1, padding: '9px', borderRadius: '8px', background: '#fff', color: '#000', border: 'none', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
            📄 Tailor resume
          </button>
          <button
            onClick={e => { e.stopPropagation(); setShowWorkspace(true) }}
            style={{ flex: 1, padding: '9px', borderRadius: '8px', background: CARD2, color: 'rgba(255,255,255,0.7)', border: `1px solid ${BORDER}`, fontSize: '13px', cursor: 'pointer' }}>
            ✉ Cover letter
          </button>
          {job.source_link && (
